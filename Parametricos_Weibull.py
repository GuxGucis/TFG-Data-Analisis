...
for i in range(len(mice)):
    for j in range(len(umbral)):
        ...
        df['duration'] = (df['max_fechatoma'] - df['fechatoma']).dt.days
        df['duration'] = df['duration'] + 1  # Agregamos 1 día a todas las duraciones para asegurarnos de que sean positivas
        df['event'] = df['FGE2'] < threshold_value
        ...
        # Verificar las columnas en aft.summary
        print("Columnas disponibles en aft.summary EKFC:", aft.summary.columns)
        print("Contenido de aft.summary EKFC:", aft.summary)
        ...
        plt.title('Curvas de Supervivencia por Franjas de Edades con umbral FGE EKFC' + str(threshold_value))
        plt.xlabel('Tiempo (días)')
        plt.ylabel('Probabilidad de Supervivencia')
        plt.legend(title='Grupo de Edad')
        plt.savefig(baseurl+'Graficas/python/ParamWeibull/PW2_Surv_edad_' + str(mice[i]) + '_umbral_' + str(threshold_value) + '.png')
        plt.close()
        ...
        plt.title('Curvas de Supervivencia por Tratamiento de Hemodialisis con umbral FGE EKFC' + str(threshold_value))
        plt.xlabel('Tiempo (días)')
        plt.ylabel('Probabilidad de Supervivencia')
        plt.legend(title='Tratamiento de Hemodialisis')
        plt.savefig(baseurl+'Graficas/python/ParamWeibull/PW2_Surv_HM_' + str(mice[i]) + '_umbral_' + str(threshold_value) + '.png')
        plt.close()

        # Verificar las columnas y el contenido de aft.summary
        summary_df = aft.summary.reset_index()  # Aplanar el índice
        print("Contenido de summary_df después de reset_index EKFC:", summary_df)

        # Filtrar los coeficientes relevantes
        summary_df = summary_df[summary_df['param'] == 'lambda_']
        print("Contenido de summary_df después del filtrado EKFC:", summary_df)

        # Graficar la importancia de las variables
        plt.figure(figsize=(12, 12))
        summary_df.set_index('covariate')['coef'].plot(kind='barh', color='skyblue', edgecolor='black')
        plt.title('Importancia de las Variables en el Modelo Weibull con umbral FGE EKFC' + str(threshold_value))
        plt.xlabel('Coeficiente')
        plt.ylabel('Variable')
        plt.axvline(x=0, color='black', linestyle='--')
        plt.subplots_adjust(left=0.3)  # Aumentar el margen izquierdo
        plt.savefig(baseurl+'Graficas/python/ParamWeibull/PW2_Importancia_Variables_' + str(mice[i]) + '_umbral_' + str(threshold_value) + '.png')
        plt.close()

        # Mostrar la significancia de las variables
        plt.figure(figsize=(12, 12))
        summary_df.set_index('covariate')['-log2(p)'].plot(kind='barh', color='lightcoral', edgecolor='black')
        plt.title('Significancia de las Variables en el Modelo Weibull con umbral FGE EKFC' + str(threshold_value))
        plt.xlabel('-log2(p)')
        plt.ylabel('Variable')
        plt.axvline(x=-log2(0.05), color='black', linestyle='--', label='p=0.05')
        plt.legend()
        plt.subplots_adjust(left=0.3)  # Aumentar el margen izquierdo
        plt.savefig(baseurl+'Graficas/python/ParamWeibull/PW2_Significancia_Variables_' + str(mice[i]) + '_umbral_' + str(threshold_value) + '.png')
        plt.close()

        print('fin de la iteración: ' + str(mice[i]) + '. En el umbral: ' + str(threshold_value))

print('----------------------------- FIN DE PARAMETRICOS DE WEIBULL ----------------------------------------')