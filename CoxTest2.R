...
significancia <- data.frame(variable = names(ANALITIC_1)[6:ncol(ANALITIC_1)])
...
for (i in mice){
  ...
  covariables <- names(ANALITIC)[9:ncol(ANALITIC)] # Asume que las columnas de interés empiezan en la 6ª posición
  ...
  covariables <- names(ANALITIC_hm)[5:ncol(ANALITIC_hm)]
  ...
  covariables <- names(ANALITIC_NN)[5:ncol(ANALITIC_NN)]
