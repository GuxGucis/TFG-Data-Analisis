...
for i in mice:
    ...
    # Eliminar columnas con baja variabilidad: 'DENSIDAD'
    # Definir las covariantes explícitamente
    covariates = [
        ...
        'Creatinina', 'Creatinina.orina', 'Fosfatasa.alcalina', 'Gamma.GT',
    ...
    # Seleccionar solo las columnas que son covariantes
    df = df[['ID', 'start', 'stop', 'estado_siguiente'] + covariates]
...
for i in mice:
    ...
    df['estado_actual'] = df.apply(lambda row: definir_estado(row['FGE2'], row['Fallecido']), axis=1)
    df['estado_siguiente'] = df.groupby('ID')['estado_actual'].shift(-1)
    ...
    print(f"Datos con hemodiálisis (EKFC): {len(hemodialisis_si)} filas")
    print(f"Datos sin hemodiálisis (EKFC): {len(hemodialisis_no)} filas")
    ...
    # Supervivencia para pacientes con hemodiálisis
    if not hemodialisis_si.empty:
        print("Ajustando el modelo Kaplan-Meier para pacientes con hemodiálisis (EKFC)")
        kmf.fit(durations=hemodialisis_si['stop'], event_observed=hemodialisis_si['estado_siguiente'] == 6, entry=hemodialisis_si['start'])
        kmf.plot_survival_function(ci_show=False, label='Hemodialisis SI')

    # Supervivencia para pacientes sin hemodiálisis
    if not hemodialisis_no.empty:
        print("Ajustando el modelo Kaplan-Meier para pacientes sin hemodiálisis (EKFC)")
        kmf.fit(durations=hemodialisis_no['stop'], event_observed=hemodialisis_no['estado_siguiente'] == 6, entry=hemodialisis_no['start'])
        kmf.plot_survival_function(ci_show=False, label='Hemodialisis NO')

    plt.title('Curva de Supervivencia Predicha por Hemodialisis (EKFC)')
    plt.xlabel('Tiempo')
    plt.ylabel('Probabilidad de Supervivencia')
    plt.legend()
    plt.grid(True)
    plt.savefig(baseurl + 'Graficas/python/MultiEstado/MS2_Surv_HM_' + str(i) + '.png')
    plt.close()
    ...
    plt.title('Curva de Supervivencia Predicha por Grupo de Edad (EKFC)')
    plt.xlabel('Tiempo')
    plt.ylabel('Probabilidad de Supervivencia')
    plt.legend()
    plt.grid(True)
    plt.savefig(baseurl + 'Graficas/python/MultiEstado/MS2_Surv_edad_' + str(i) + '.png')
    plt.close()

    print('fin de la iteración (EKFC): ' + str(i))

print('--------------------------- FIN DE MULTIESTADO ------------------------------------')
#%%
